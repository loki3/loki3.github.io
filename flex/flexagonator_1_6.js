...
(function (Flexagonator) {
    function isGroupCycleError(result) {
        return result && result.findCycleError !== undefined;
    }
...
(function (Flexagonator) {
    ...
    class FindCyclesFromStates {
         ...
         */
        constructor(states, start, flexes) {
            this.states = states;
            this.start = start;
            this.flexes = flexes;
            this.error = null;
        ...
        findCycles(sequence) {
            ...
            const fm = new Flexagonator.FlexagonManager(f1, undefined, this.flexes);
        ...
        checkCycle(fm, sequence) {
            ...
            if (check <= 2) {
                ...
                const flexNames = Flexagonator.parseFlexSequence(withoutRotates);
                if (flexNames.length < 2) {
                    return;
                }
            }
            const normalized = Flexagonator.normalizeSequence(sequence, this.numPats);
            this.cycles.push({ sequence: normalized, cycleLength: check });
        }
    }
    Flexagonator.FindCyclesFromStates = FindCyclesFromStates;
    /**
...
(function (Flexagonator) {
     ...
     */
    class FindShortest {
        static make(start, end, flexes, right, over, startDirs, endDirs) {
            const startFlexagon = Flexagonator.Flexagon.makeFromTree(start, undefined, startDirs);
            if (Flexagonator.isTreeError(startFlexagon)) {
                return startFlexagon;
            }
            const endFlexagon = Flexagonator.Flexagon.makeFromTree(end, undefined, endDirs !== null && endDirs !== void 0 ? endDirs : startDirs);
        ...
        getFlexes(normalize) {
            ...
            return Flexagonator.normalizeSequence(flexes, numPats);
        }
...
(function (Flexagonator) {
     ...
     */
    function getStateToStateTree(allRelFlexes, startAt) {
        startAt = startAt !== null && startAt !== void 0 ? startAt : 0;
        const result = [];
        const found = new Set();
        const queued = [startAt];
        let which = 0;
        found.add(startAt);
        do {
    ...
    Flexagonator.getStateToStateTree = getStateToStateTree;
    /**
     * take a description of all the flexes that can be performed from each state
...
(function (Flexagonator) {
    ...
    */
     ...
     */
    function dotTree(allRelFlexes, startAt) {
        const transitions = Flexagonator.getStateToStateTree(allRelFlexes, startAt);
        return dotSimpleGraph(transitions);
    }
    Flexagonator.dotTree = dotTree;
...
(function (Flexagonator) {
    ...
    class RelativeFlex {
        ...
        toString(numPats) {
            const sequence = numPats === undefined ? this.getSequence() : Flexagonator.normalizeSequence(this.getSequence(), numPats);
    ...
    function relativeFlexesToString(flexes, numPats) {
...
(function (Flexagonator) {
    function makeAtomicFlex(name, inPattern, outPattern) {
        ...
        if (Flexagonator.isAtomicParseError(input)) {
            return Object.assign(Object.assign({}, input), { flexName: name });
        }
        const output = Flexagonator.stringToAtomicPattern(outPattern);
        if (Flexagonator.isAtomicParseError(output)) {
            return Object.assign(Object.assign({}, output), { flexName: name });
        }
...
(function (Flexagonator) {
    ...
    function isAtomicParseError(result) {
        return (result !== null) && (result !== undefined) && result.atomicParseCode !== undefined;
    }
...
(function (Flexagonator) {
    ...
    function convertAtomicFlex(name, patCount, inPattern, outPattern, rotation) {
        const atomicFlex = Flexagonator.makeAtomicFlex(name, inPattern, outPattern);
        if (Flexagonator.isAtomicParseError(atomicFlex)) {
            return atomicFlex;
        }
        ...
        const modifiedCountIn = Flexagonator.getPatsCount(atomicFlex.input.left) + Flexagonator.getPatsCount(atomicFlex.input.right);
        const modifiedCountOut = Flexagonator.getPatsCount(atomicFlex.output.left) + Flexagonator.getPatsCount(atomicFlex.output.right);
        const totalLeavesIn = Flexagonator.getLeafCount(atomicFlex.input.left) + Flexagonator.getLeafCount(atomicFlex.input.right);
        const totalLeavesOut = Flexagonator.getLeafCount(atomicFlex.output.left) + Flexagonator.getLeafCount(atomicFlex.output.right);
        if (modifiedCountIn !== modifiedCountOut || totalLeavesIn !== totalLeavesOut) {
            return { reason: Flexagonator.FlexCode.SizeMismatch, flexName: name };
        }
        const flex = Flexagonator.createLocalFlex(name, patCount - modifiedCountIn, totalLeavesIn + 1, inPatLeft, inPatRight, outPatLeft, outPatRight, inDirLeft, inDirRight, outDirLeft, outDirRight, rotation);
        return flex;
    }
    Flexagonator.convertAtomicFlex = convertAtomicFlex;
    function convertPats(pattern, side) {
        var _a;
        const connected = side === 'left'
            ? (_a = pattern.left) === null || _a === void 0 ? void 0 : _a.slice().reverse()
            : pattern.right;
...
(function (Flexagonator) {
    ...
    function addFullFlexes(flexes) {
        ...
        // start & end /\
        flexes["N"] = Flexagonator.makeAtomicFlex("n", "a [-2,1] / # [-3,[-5,4]] \\ b", "a [[[3,-2],-4],1] / # -5 \\ b");
        // start & end //\
        flexes["Bfl"] = Flexagonator.makeAtomicFlex("boffle", "a 1 / # [-3,2] / [-4,[-6,5]] \\ b", "a [[3,-4],[1,-2]] / # -5 / -6 \\ b");
        // from \\ to \\
...
(function (Flexagonator) {
    ...
    function makeMorphFlexes(patCount) {
        ...
        // /\
        flexes["N"] = Flexagonator.createLocalFlex("n", patCount - 2, 6, [[-2, 1]], /**/ [[-3, [-5, 4]]], [[[[3, -2], -4], 1]], /**/ [-5], "/", "|", "/", "|");
        // //\
        flexes["Bfl"] = Flexagonator.createLocalFlex("boffle", patCount - 3, 7, [1], /**/ [[-3, 2], [-4, [-6, 5]]], [[[3, -4], [1, -2]]], /**/ [-5, -6], "/", "/|", "/", "/|");
        // backflip = Mkf' Mkb
        ...
        flexes["Tr3"] = Flexagonator.createLocalFlex("transfer 3", patCount - 3, 6, [1, 2], /**/ [[[-4, 5], 3]], [[3, [1, -2]]], /**/ [4, 5], "/|", "/", "/", "|/", Flexagonator.FlexRotation.BAC); // /\#/ -> /#\/
        flexes["Tr3s"] = Flexagonator.createLocalFlex("transfer 3 stable", patCount - 3, 6, [1, 2], /**/ [[[-4, 5], 3]], [[3, [1, -2]], 4], /**/ [5], // keep same reference hinge
        "/|", "/", "/|", "/"); // /\#/
        flexes["Tr4"] = Flexagonator.createLocalFlex("transfer 4", patCount - 4, 7, [1, 2], /**/ [3, [4, [6, -5]]], [[[-2, 1], 3], 4], /**/ [5, 6], "|/", "/|", "|/", "/|"); // \/#/\
        flexes["Tr6"] = Flexagonator.createLocalFlex("transfer 6", patCount - 6, 9, [1, 2, 3], /**/ [4, 5, [6, [8, -7]]], [[[-2, 1], 3], 4, 5], /**/ [6, 7, 8], "|//", "//|", "|//", "//|"); // \//#//\
        ...
        if (patCount >= 7) {
            ...
            if (patCount === 10) {
                ...
                flexes["Tao"] = Flexagonator.createLocalFlex("three-and-open", patCount - 7, 10, [[-2, 1], -3, -4], /**/ [[6, -5], 7, 8, 9], [1, 2, 3], /**/ [[-5, 4], -6, -7, [9, -8]], "/|/", "///|", "|//", "//|/", Flexagonator.FlexRotation.BAC); // /\/#///\  ->  \//#//\/
            }
        }
        // add all the inverses
        for (const flex of Object.keys(flexes)) {
...
var Flexagonator;
(function (Flexagonator) {
    function drawStrip(paint, leaflines, content, props, scale, rotation, captions, center, layer) {
        const [w, h] = paint.getSize();
    ...
    function drawUnfoldedObjects(target, objects, options) {
        ...
        for (let i = 0; i < slices.paints.length; i++) {
            ...
            if (paint && opt) {
                ...
                const leaflinesSubset = Flexagonator.sliceLeafLines(leaflines, opt.start, opt.end, cutEnds);
                paint.start();
                Flexagonator.drawStrip(paint, leaflinesSubset, content, objects.leafProps, opt.scale, opt.rotation, opt.captions, undefined, opt.layer);
                paint.end();
            }
        }
    }
...
(function (Flexagonator) {
    ...
    function isError(result) {
        return isTreeError(result) || isPatternError(result) || isFlexError(result) || Flexagonator.isAtomicParseError(result);
    }
    ...
    function errorToString(error) {
        ...
        else if (Flexagonator.isAtomicParseError(error)) {
    ...
    */
    ...
    (function (TreeCode) {
        ...
        TreeCode["InvalidPats"] = "invalid pat specification";
    ...
    function isTreeError(result) {
        return result && result.reason !== undefined && result.context !== undefined;
    }
            ...
            if (outputDirs) {
                ...
                for (let i = 0; i < rawDirs.length; i++) {
                    ...
                    if (flexDir === null || flexDir === undefined) {
                        ...
                        this.indices[i] = which !== null && which !== void 0 ? which : i;
                    }
                }
            }
        }
...
(function (Flexagonator) {
    ...
    function normalizeFlexName(name) {
        return name.replace(/’/g, "'");
    }
    Flexagonator.normalizeFlexName = normalizeFlexName;
    /**
...
(function (Flexagonator) {
    ...
    */
    class FlexagonManager {
        ...
        getFlexHistoryString() {
            const flexes = this.history.getCurrent().flexes;
            return Flexagonator.sequenceToString(flexes);
        }
...
(function (Flexagonator) {
     ...
     */
    function labelAsTree(flexagon, colors, matching) {
    ...
    function labelInsides(leafToLabel, flexagon, matching) {
    ...
    function handlePatMatching(leafToLabel, pat) {
...
(function (Flexagonator) {
    ...
    function makeFlexFromSequence(sequence, flexes, name, rotation, inputDirs, outputDirs, orderOfDirs) {
        const dirs = inputDirs ? Flexagonator.Directions.make(inputDirs) : undefined;
    ...
    function makeEmptyFlexagon(flexes, dirs) {
        const allNames = Object.getOwnPropertyNames(flexes);
        ...
        return Flexagonator.Flexagon.makeFromTree(pats, undefined, dirs);
    }
...
(function (Flexagonator) {
    ...
    const primes = [
        ...
        "Bf", "N", "N'", "Tr2", "Tr3", "Tr4", "Tr6", "Tao", // local: directions other than ////
        "Ds", "Tu", "Hat", "Fet", // global: directions other than ////
    ];
    const unique = [
        "S3", "St", "Fm", "F3", "F4",
        "Lbf", "Lbb", "Lh", "Lk", "L3",
    ...
    function createTwist(patCount, dirs) {
        ...
        return Flexagonator.makeFlex("twist flex", input, output, Flexagonator.FlexRotation.BAC, dirs, dirs);
    }
...
(function (Flexagonator) {
    ...
    function getPatsPrefix(word) {
        ...
        return typeof asNumber === 'number' && asNumber > 0 ? asNumber : prefix;
    }
})(Flexagonator || (Flexagonator = {}));
var Flexagonator;
(function (Flexagonator) {
    ...
    function namePiecesToScript(name) {
        // normalize
        if (name.overallShape === "") {
            name = Object.assign(Object.assign({}, name), { overallShape: undefined });
        }
        if (name.leafShape === "") {
            name = Object.assign(Object.assign({}, name), { leafShape: undefined });
        }
        if (name.patsPrefix === "") {
            name = Object.assign(Object.assign({}, name), { patsPrefix: undefined });
        }
    ...
    function overallShapeToScript(overallShape, n, leafShape) {
        ...
        if (overallShape.endsWith('ring') && (sides !== null || overallShape === 'star ring')) {
            ...
            if (sides === null && n === 20 && (leafShape === undefined || leafShape.startsWith('right'))) {
                const directions = Flexagonator.Directions.make('/||/'.repeat(5));
            ...
            if (sides === null && n === 24 && (leafShape === undefined || leafShape.startsWith('bronze'))) {
                const directions = Flexagonator.Directions.make('/||/'.repeat(6));
        ...
        if (sides === 6 && (!leafShape || leafShape === 'regular') && n >= 10 && (n - 6) % 4 === 0) {
            ...
            return { angles2: [60, 60], directions: Flexagonator.Directions.make(dirs.repeat(2)) };
        }
...
(function (Flexagonator) {
    ...
    function normalizeSequence(flexes, numPats) {
        const flexNames = Flexagonator.parseFlexSequence(flexes);
        let names = [];
        for (const name of flexNames) {
            names = Flexagonator.addAndConsolidate(names, [name], numPats);
        }
        const result = sequenceToString(names);
        return result;
    }
    Flexagonator.normalizeSequence = normalizeSequence;
    /** standardize spacing in a flex sequence */
    function sequenceToString(flexes) {
    ...
    Flexagonator.sequenceToString = sequenceToString;
})(Flexagonator || (Flexagonator = {}));
var Flexagonator;
(function (Flexagonator) {
...
(function (Flexagonator) {
    ...
    function runScriptItem(fm, item) {
        ...
        if (item.pats !== undefined) {
            ...
            const pats = typeof item.pats !== 'string' ? item.pats
                : Flexagonator.parsePats(item.pats, fm.flexagon.getPatCount());
            if (!pats) {
                return { reason: Flexagonator.TreeCode.InvalidPats, context: item.pats };
            }
            const result = Flexagonator.Flexagon.makeFromTreeCheckZeros(pats);
        ...
        if (item.labelAsTreeMatch !== undefined) {
            const props = Flexagonator.labelAsTree(fm.flexagon, item.labelAsTreeMatch, true);
            fm.leafProps = props;
        }
        if (item.addFlex !== undefined) {
            const f = item.addFlex;
            const newFlex = makeFlexFromScript(f, fm);
            if (Flexagonator.isError(newFlex)) {
                return newFlex;
            }
    ...
    function makeFlexFromScript(f, fm) {
        const name = f.name ? f.name : f.shorthand;
        const fr = angleOrderToFlexRotation(f.rotation);
        if (Flexagonator.isFlexFromSequence(f)) {
            return Flexagonator.makeFlexFromSequence(f.sequence, fm.allFlexes, name, fr, f.inputDirs, f.outputDirs, f.orderOfDirs);
        }
        if (typeof (f.input) === 'string' && typeof (f.output) === 'string') {
            return Flexagonator.convertAtomicFlex(name, fm.flexagon.getPatCount(), f.input, f.output, fr);
        }
        if (typeof (f.input) !== 'string' && typeof (f.output) !== 'string') {
            return Flexagonator.makeFlex(name, f.input, f.output, fr, f.inputDirs, f.outputDirs, f.orderOfDirs);
        }
        return typeof (f.input) === 'string' ? { reason: Flexagonator.FlexCode.BadFlexInput } : { reason: Flexagonator.FlexCode.BadFlexOutput };
    }
})(Flexagonator || (Flexagonator = {}));
var Flexagonator;
(function (Flexagonator) {
...
(function (Flexagonator) {
    ...
    */
    ...
    function sliceLeafLines(leaflines, start, end, cutEnds) {
    ...
    function getFoldsCutEnds(leaflines, start, end, cutEnds) {
        if (cutEnds !== true) {
    ...
    function getCutsCutEnds(leaflines, start, end, cutEnds) {
        const cuts = leaflines.cuts.slice(start, end);
        if (cutEnds !== true) {
            return cuts;
        }
        // add in first & last folds for this slice
        const first = leaflines.folds[start !== null && start !== void 0 ? start : 0];
        const last = leaflines.folds[end !== null && end !== void 0 ? end : leaflines.folds.length - 1];
